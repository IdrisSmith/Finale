...
function createShot(){
   var x = getX(player)
 var y = getY(player)
 shot.setAttribute("y", y + 10)
  shot.setAttribute("x", x + 15)
  if(shooting == false){
  shoot();
}
}
...
function enemyGeneration(){
  randomX = Math.random() * 180
    enemy.setAttribute("x", randomX)
  enemy.setAttribute("y", -20)
  if (!gameStarted) {
  moveEnemy();
    gameStarted = true
  ...
  if(enemyFall  == false){
  setTimeout(enemyGeneration, 5000)
  }
}
...
function gameOver(){
  if(death){
screen.setAttribute("opacity", 1)
var highScore = makeText("Congratulations! You were able to kill " + score + " of them!", 0, 100, 10, "", "white")
  }
}
enemyGeneration();
